<?php
/**
* Contact Us Page on Super Admin
*
* @author KCG
* @since July 14, 2017
* @version 1.0
	...
	<div class="portlet light">
	    ...
	    <div class="portlet-body">
	    	...
	    	<div class="row">
	    		...
	    		<div class="col-md-8">
	    			<div class="sub-title">{{ $contact_us->subject }}</div>
				            ...
				            @foreach ($messages as $message)
				            	<?php 
				            		...
				            		if ($message->sender_id)
				            			...
				            			$sender->fullname = $message->contact_us->fullname;
				            			$sender->email    = $message->contact_us->email;
				            		}

				            		if ($message->sender_id == $current_user->id) {
				            			$receiver->fullname = $message->contact_us->fullname;
				            			$receiver->email    = $message->contact_us->email;
				            		} else {
								            ...
								            <div class="col-md-7">
								                ...
								                <div class="sender-names">
								                    <div class="sender-role">
								                    	{!! $sender->id == $current_user->id?'me':$sender->fullname !!} ({{ $sender->email }})
								                   	</div>
								                </div>
								                <div class="receiver-names">
								                    to {!! $receiver->id == $current_user->id?'me':$receiver->fullname !!} ({{ $receiver->email }})
								                </div>
								            </div>
								            <div class="col-md-5">
								                <div class="send-date pull-right">{{ format_date('M j, Y, g:i A', $message->created_at) }}({{ ago($message->created_at) }})</div>
								            </div>
								        </div>
				        ...
				        <form id="form_message" action="{{ route('admin.super.contact_us.send', ['id' => $contact_us->id]) }}" class="form-horizontal" method="post">
