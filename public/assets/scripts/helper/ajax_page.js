(function ($) {
	$.ajaxPage = {
		...
		replaceHTML: function($container, html) {
			var $html = $(html);
            var $contents = $($.ajaxPage.selector($container), $html);

            if ($.ajaxPage.selector($container) == $.ajaxPage.selector($html))
	...
	$.fn.ajaxPage = function (params) {
		var options = $.extend({}, $.fn.ajaxPage.defaults, params);
		...
		return this.each(function() {
			...
			function submitForm($form) {
				...
				$form.trigger('ajaxPage.before.submit', [$self]);
				
				var container_id = $form.data('container');
	                ...
	                success: function(html) {
	                	if (typeof html == 'string') {
		                	if (container_id && container_id != '')
		                		$self = $.ajaxPage.replaceHTML($(container_id), html);
		                	else
		                		$self = $.ajaxPage.replaceHTML($self, html);
	                	}

	                	options.success(html, $self);

	                    $form = $.ajaxPage.form($self);
	                    $('body').trigger('ajaxPage.success', [$self, $form, html]);
			...
			$('ul.pagination li a', $(this)).off('click');
	...
	$.fn.ajaxPage.defaults = { 'success': function(html) {}, 'sort': true };
